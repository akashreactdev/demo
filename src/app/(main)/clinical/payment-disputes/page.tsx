...
const PaymentDisputes: React.FC = () => {
  ...
  const handleOnclick = (ele: PassportItem) => {
    localStorage.setItem("selectClinicalPaymentDispute", JSON.stringify(ele));
    navigateWithLoading(`/clinical/payment-disputes/${ele._id}`);
  };
      ...
      <Box mt={3}>
        <CommonCard>
          ...
          <Typography variant="caption" fontWeight={400}>
            This displays payments from Zorbee to a clinical that are awaiting
            approval.
          </Typography>
            ...
            {passportListData.map((ele, index) => {
              return (
                <Box key={index}>
                  <ApprovalListItem
                    profilePic={
                      ele?.userId?.profile != "image/clinical" &&
