...
export interface AddResourcesResponse {
  data: {
    success: boolean;
    message: string;
  };
...
const schema: yup.ObjectSchema<FormData> = yup.object({
  ...
  articleCategory: yup
    ...
    .required("Required"),
  thumbnailFile: yup.string().required("Required"),
  sections: yup
    .array()
    .of(
      yup.object({
...
const AddNewArticle: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (previewData) {
      try {
        const resource: PreviewResources = JSON.parse(previewData);
        ...
        const mappedSections =
          resource.article?.map((section: ArticleItem, i: number) => {
            ...
            if (section.subText)
              items.push({
                id: `item-${Date.now()}-${i}-3`,
                type: "subtext",
                subTitle: section.subText,
              });
            if (section.caption)
              items.push({
                id: `item-${Date.now()}-${i}-4`,
                type: "caption",
                caption: section.caption,
              });
            if (section.imageUrl)
              items.push({
                id: `item-${Date.now()}-${i}-5`,
                type: "image",
                imageUrl: section.imageUrl,
              });

            return { id: `section-${Date.now()}-${i}`, items };
          }) ?? [];

        replaceSections(mappedSections);
        setValue("sections", mappedSections);

        if (resource.userType && Array.isArray(resource.userType)) {
          const userTypeArray = resource.userType;
          const allUserTypes = [2, 3, 4, 5];
          const hasAllUserTypes =
            userTypeArray.length === 4 &&
            allUserTypes.every((type) => userTypeArray.includes(type));

          if (hasAllUserTypes) {
            setValue("userType", 1);
          } else if (userTypeArray.length === 1) {
            setValue("userType", userTypeArray[0]);
          } else {
            setValue("userType", userTypeArray[0] || null);
          }
        }
    ...
    try {
      ...
      if (response?.data?.success) {
        ...
        if (resource.userType && Array.isArray(resource.userType)) {
          ...
          // Check if it contains all user types [2, 3, 4, 5]
          const allUserTypes = [2, 3, 4, 5];
          const hasAllUserTypes =
            userTypeArray.length === 4 &&
            allUserTypes.every((type) => userTypeArray.includes(type));
          ...
          if (hasAllUserTypes) {
            // If all user types are present, set to "All" (value: 1)
            setValue("userType", 1);
          } else if (userTypeArray.length === 1) {
            // If single value, set that value directly
            setValue("userType", userTypeArray[0]);
          } else {
            // Default case - set first value or null
            setValue("userType", userTypeArray[0] || null);
          }
        }
  ...
  const handlePreviewClick = async () => {
    ...
    if (isValid) {
      ...
      if (id) {
        navigateWithLoading(
          `/support/resources/preview-article?id=${id}`
        );
      } else {
        navigateWithLoading(`/support/resources/preview-article`);
      }
    } else {
    }
  };
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      //   toast.error("Failed add article.");
      // }
