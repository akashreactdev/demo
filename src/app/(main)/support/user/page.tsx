...
const User: React.FC = () => {
  ...
  const handleActionItemClick = (row: SupportData, idx: number) => {
    if (idx === 0) {
      navigateWithLoading(`/support/user/profile/${row._id}`);
      handleClose();
    }
    if (idx === 1) {
  ...
  const handleEventSet = async () => {
    try {
      ...
      if (response?.data?.success) {
        ...
        handleEventClose();
        handleClose();
        fetchCarerSupport(currentPage, 2, lastFilters?.supportTicketStatus, lastSearchValue);
      }
    } catch (e) {
      console.log(e);
    }
    ...
    <Box>
      <CommonCard>
        ...
        <Typography fontSize={"12px"}>
          This displays all logged user support tickets.
        </Typography>
      </CommonCard>
