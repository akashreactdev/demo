...
const ProfileRequest = () => {
  ...
  useEffect(() => {
    setIsLoading(false);
    const CarerData = localStorage.getItem("selectProviderPaymentDispute");
    if (CarerData) {
      try {
        ...
        <CommonCard>
          ...
          <Box mt={3}>
            <AgreementStatus
              onClick={() =>
                navigateWithLoading(
                  `/providers/payment-disputes/${params?.name}/service-agreement/${singlePassportData?.dispute?.agreementId?._id}`
                )
              }
            >
              <Stack
                      ...
                      {singlePassportData?.notes?.length ? (
                        singlePassportData?.notes.map((note, index) => (
                          <RequestCard
                            ...
                            onClickRightButton={() =>
                              navigateWithLoading(
                                `/providers/payment-disputes/${params?.name}/care-notes/${note._id}`
                              )
                            }
                          />
                        ))
                    ...
                    {singlePassportData?.visitLogs?.length ? (
                      singlePassportData?.visitLogs.map((visit) => (
                        <VisitLogsCard
                          ...
                          onClick={() =>
                            navigateWithLoading(
                              `/providers/payment-disputes/${params?.name}/visit-log/${visit._id}`
                            )
                          }
                        />
                      ))
