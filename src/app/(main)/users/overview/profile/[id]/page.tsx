...
import { getSingleUserInfo, updateUserInfo } from "@/services/api/usersApi";
...
import { UserPermissionsState } from "../../page";
...
const Profile = () => {
  ...
  const cardData = [
    {
      ...
      redirectionRoute: `/users/overview/profile/${params?.id}/care-plan`,
    },
    {
      ...
      redirectionRoute: `/users/overview/profile/${params?.id}/medical-history`,
    },
    {
      ...
      redirectionRoute: `/users/overview/profile/${params?.id}/medication-log`,
    },
  ];
  ...
  useEffect(() => {
    ...
    if (savedPermissions) {
      try {
        ...
        console.error(
          "Failed to parse userPermissions from localStorage",
          err
        );
      }
    }
  ...
  const handleActionItemClick = (status: string, id: string | number) => {
    navigateWithLoading(
      `/users/overview/profile/${params?.id}/service-agreement/${id}?status=${status}`
    );
  };
  ...
  const account_information = useMemo(() => {
    ...
    return [
      ...
      },
      {
        label: "Date of birth",
        value: userInfo?.dob
          ? moment(userInfo?.dob).format("DD MMMM YYYY")
          : "N/A",
        key: "date",
        isEditable: true,
      },
  ...
  const handleActionClick = (event: React.MouseEvent<HTMLButtonElement>) => {
    if (userPermissionsState?.deactivateUserAccount) {
                ...
                userInfo?.completedAgreement?.length > 0 && (
                  <Box mt={4}>
                    <ViewAllButton
                      ...
                      onClick={() =>
                        navigateWithLoading(
                          `/users/overview/profile/${params?.id}/service-agreement`
                        )
                      }
                    />
                  </Box>
          ...
          <Box mt={4}>
            <CommonCard>
              ...
              <Box mt={3}>
                ...
                >
                  <Image
                    src={"/assets/svg/carers/profile/funding.svg"}
                    alt="user-profile-pic"
                    height={24}
                    width={24}
                  />
                  <Typography variant="body1" fontWeight={400}>
                    {userInfo?.fundingType?.name || "N/A"}
                  </Typography>
                </Stack>
              </Box>
              <Box mt={3}>
