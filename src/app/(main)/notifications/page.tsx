...
interface clearAllNotificationResponse {
  data: {
    success: boolean;
    message: string;
  };
...
const { navigateWithLoading } = useRouterLoading();
  ...
  const handleClearAllNotifications = async () => {
    try {
      const response =
        (await clearAllNotification()) as clearAllNotificationResponse;
      if (response?.data?.success) {
        toast.success(response?.data?.message);
        fetchAllNotifications();
      }
        ...
        >
          ...
          >
            <Box
              sx={{ cursor: "pointer" }}
              onClick={() =>
                navigateWithLoading("/notifications/archive-notifications")
              }
            >
